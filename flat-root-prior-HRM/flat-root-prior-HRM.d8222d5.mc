...
let g: all c10. all b30. all a88. (a88 -> b30 -> c10 -> a88) -> (a88, [b30]) -> c10 -> (a88, [b30]) =
  lam f32.
    lam acc18: (a88, [b30]).
      lam x212.
        match acc18 with (acc19, [ x176 ] ++ xs1)
        then
          (f32 acc19 x176 x212, xs1)
        else
...
in
let anon4: Mat Float -> Mat Float = lam x172.
    x172 in
...
in
let anon6: Mat Float -> Mat Float = lam x171.
    x171 in
...
in
let anon8: Mat Float -> Mat Float = lam x170.
    x170 in
...
in
let matMap: all x169. (Float -> Float) -> Mat Float -> Mat Float =
  lam f24: Float -> Float.
    lam mtx12: Mat Float.
...
let x: all #var"B13". all #var"A13". (Int -> #var"A13" -> #var"B13") -> Int -> #var"A13" -> #var"B13" =
  lam f22.
    lam i14.
      lam x167: #var"A13".
        let x168: #var"B13" = f22 (addi i14 1) x167 in
        x168
in
let x1: all #var"B12". all #var"A12". (#var"A12" -> #var"B12" -> #var"A12") -> #var"A12" -> #var"B12" -> #var"A12" =
  lam f21.
    lam a44.
      lam b11: #var"B12".
        let x166: #var"A12" = f21 a44 b11 in
        x166
in
let x2: all #var"B11". all #var"A11". (#var"A11" -> Int -> #var"B11" -> #var"A11") -> #var"A11" -> Int -> #var"B11" -> #var"A11" =
  lam f20.
    lam a43.
      lam idx10.
        lam b10: #var"B11".
          let x165: #var"A11" = f20 a43 (addi idx10 1) b10 in
          x165
in
...
let ifCont1 = lam #var"8": Int.
    0. in
let exp1: Float -> Float = lam x162: Float.
    mathExp x162 in
let log1: Float -> Float = lam x161: Float.
    mathLog x161 in
...
let anon15: all #var"X10". (Int -> #var"X10") -> Int -> #var"X10" =
  lam f17.
    lam idx8: Int.
      let x160: #var"X10" = f17 (addi idx8 1) in
      x160
in
...
let anon21: Float -> Float -> Float =
  lam sum1.
    lam x157: Float.
      let x158: Float = divf x157 sum1 in
      x158
in
...
let anon24: Int -> Int -> Float =
  lam index1.
    lam i12: Int.
      let x155: Float = bool2real (eqi index1 i12) in
      x155
in
...
let anon26: Float -> [Float] -> [Float] =
  lam meanDist1.
    lam s5: [Float].
      let x153: [Float] = sapply s5 (anon25 meanDist1) in
      x153
in
...
let categoricalLogPmf: Int -> [Float] -> Float =
  lam x152: Int.
    lam param6: [Float].
      match
        match leqi 0 x152 with true
        then
          lti x152 (length1 param6)
        else
          false
      with
        true
...
let anon28: Matrix Float -> Int -> Int -> Float =
  lam mtx1.
    lam i11.
      lam j: Int.
        let x151: Float = mtxGet i11 j mtx1 in
        x151
in
...
let anon29: Matrix Float -> Int -> Int -> Float =
  lam samplingProb2.
    lam h4: Int.
      lam obs: Int.
        let x150: Float = categoricalLogPmf obs (hostMsgToProb samplingProb2 h4)
        in
        x150
in
let getRepertoireSamplingDensity: [Int] -> Matrix Float -> Float =
  lam x149: [Int].
    lam samplingProb1: Matrix Float.
      seqSumReal (sapplyi x149 (anon29 samplingProb1))
in
                    ...
                    addf
                      ...
                      (hostIndepLikelihood
                         ...
                         (let target73 = modelParams6 in
                          match target73 with ModelParams1 x130
                          then
                            x130.embeddedQMatrix
                          else
                            let #var"2" =
                              print
...
let anon39: Float -> Float -> Float =
  lam acc4: Float.
    lam val: Float.
      let x128: Float = addf acc4 val in
      x128
in
let anon40: Matrix Float -> Int -> Int -> Float =
  lam kernel1.
    lam fromState: Int.
      lam toState: Int.
        let x127: Float = log1 (mtxGet (addi fromState 1) (addi toState 1) kernel1)
        in
        x127
in
...
let simplexMove: [Float] -> Float -> Float -> Dist([Float]) =
  lam x125: [Float].
    lam alpha1: Float.
      lam epsilon: Float.
        let newParams = sapply x125 (newParamFun alpha1 epsilon) in
        Dirichlet newParams
in
let scaleMove: Float -> Float -> Dist(Float) =
  lam x124: Float.
    lam lambda6: Float.
      Reciprocal
        (mulf x124 (exp1 (divf (negf lambda6) 2.)))
        (mulf x124 (exp1 (divf lambda6 2.)))
in
    ...
    lam param4: [Float].
      lam lambda5: Float.
        lam errMargin1: Float.
          let currProb = get param4 (subi (addi x121 1) 1) in
          ...
          then
            ...
            let newParam =
              sapply
                ...
                (anon42 x121 param4 currProb nextProb)
            in
...
let rbLambdaMove: [Float] -> Dist([Float]) =
  lam x120: [Float].
    ...
    let alpha = 25. in
    simplexMove x120 alpha _EPSILON
in
let rbBetaMove: Float -> Dist(Float) =
  lam x119: Float.
    let lambda4 = 1. in
    scaleMove x119 lambda4
in
let rbMuMove: Float -> Dist(Float) =
  lam x118: Float.
    let lambda3 = 0.2 in
    scaleMove x118 lambda3
in
let categoricalMove: Int -> [Float] -> Dist(Int) =
  lam x117: Int.
    lam param3: [Float].
      ...
      categoricalShiftKernel x117 param3 lambda2 errMargin
in
type ReturnType in
type Bridge in
...
in
let anon51: {interactions: [[Int]], symbiontTree: TreeLabeled, hostDistances: [[Float]]} -> () -> ReturnType =
  lam input1.
    lam #var"5".
      hostRepertoireModel

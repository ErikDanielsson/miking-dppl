lam st.
    ...
    lam st795.
      lam a37.
        lam b5.
          ...
          in
          match
            p_subApply st806 hrmStoreSubmodel (hrmInit {}) x1249 x1251
          with
            (st807, x1252)
          in
                    ...
                    lam nEvents3.
                      lam nhosts5.
                        lam modelParams.
                          ...
                          in
                          match
                            p_subApply st114 hrmStoreSubmodel (hrmInit {}) x147 x149
                          with
                            (st115, x150)
                          in
                    ...
                    lam nEvents31.
                      lam nhosts51.
                        lam modelParams1.
                          ...
                          in
                          match
                            p_subApply st142 hrmStoreSubmodel (hrmInit {}) x189 x191
                          with
                            (st143, x192)
                          in
                             ...
                             match
                               p_traverseSeq
                                 ...
                                 (match embeddedQMatrix with EmbeddedMarkovChainMatrix1 x331
                                  then
                                    -- print "x331.transitionProbs: ";
                                    -- dprint x331.transitionProbs;
                                    -- printLn "---------------------";
                                    -- flushStdout;
                             ...
                             match
                               p_traverseSeq
                                 ...
                                 (match embeddedQMatrix1 with EmbeddedMarkovChainMatrix1 x332
                                  then
                                    -- print "x332.transitionProbs: ";
                                    -- dprint x332.transitionProbs;
                             ...
                             match
                               p_traverseSeq
                                 ...
                                 (match embeddedQMatrix2 with EmbeddedMarkovChainMatrix1 x333
                                  then
                                    -- print "x333.transitionProbs: ";
                                    -- dprint x333.transitionProbs;
                             ...
                             match
                               p_traverseSeq
                                 ...
                                 (match embeddedQMatrix3 with EmbeddedMarkovChainMatrix1 x3310
                                  then
                                    -- print "x3310.transitionProbs: ";
                                    -- dprint x3310.transitionProbs;
                                    -- printLn "---------------------";
                                    -- flushStdout;
      ...
      lam msg1.
        lam x909.
          ...
          with
            (st587, x915)
          in
          p_assume "dummy assume" st587 (hrmStoreNode nl x909) x915
  in
  recursive
    let rec8 =
      lam st277.
        lam c23.
          lam s8.
            ...
            match s8 with [ _,
                _ ] ++ _
            then
              match anon15 st277 c23 (get s8 0) nl with (st278, x420)
              in
              match splitAt s8 1 with {#label"1" = x421}
              in
              match rec8 st278 c23 x421 nl with (st279, x422)
              in
            ...
            then
              ...
              in
              let slice9 = x423 in
              match anon15 st277 c23 e10 nl with (st280, x424)
              in
              (st280, [ x424 ])
            else match s8 with ""
            in
                ...
                lam host.
                  lam nodeLabel.
                    lam embeddedQMatrix11.
                      -- let printFunc = (lam currentState. printLn (join [
                      --   "Entering sampleHostHistoryRec\n",
                      --   "Current state: ", int2string currentState,
                      --   " final state ", int2string finalState11,
                      --   " nodelabel ", int2string nodeLabel, 
                      --   " hostlabel ", int2string host
                      --   ])) in
                      -- match p_map st281 printFunc currentState with (st281, _) in
                      -- match p_apply st281 partprint finalState11 with (st281, _) in
                      p_bind
                        st281
                        hrmStoreSubmodel
                        (hrmInit {})
                           ...
                           lam x425.
                             ...
                             then
                               ...
                               p_traverseSeq
                                 ...
                                 (match embeddedQMatrix11 with EmbeddedMarkovChainMatrix1 x4010
                                  then
                                    -- print "x4010.transitionProbs: ";
                                    -- dprint x4010.transitionProbs;
                             ...
                             match
                               p_map
                                 ...
                                 (lam x432.
                                    lam x433.
                                      -- printLn (join ["st285:Previous value: ", int2string x433]);
                                      -- printLn (join ["Distribution: ", strJoin ", " (map float2string (get x432 x433))]);
                             ...
                             in
                             match p_assume (join ["branchstate0 ", int2string nodeLabel, " ", int2string host, " ", float2string x425]) st288 (hrmStoreBranchState nodeLabel host) x437 with (st289, nextState11)
                             in
                             match
                               p_traverseSeq
                                 st289
                             ...
                             in
                             match p_assume "dummy assume" st294 (hrmStoreBranchTime nodeLabel host) x446 with (st295, x447)
                             in
                             match
                               p_map
                                 st295
                ...
                lam host1.
                  lam nodeLabel1.
                    lam embeddedQMatrix12.
                      -- let printFunc = (lam currentState1. lam finalState12. printLn (join [
                      --   "Entering sampleHostHistoryRec1\n",
                      --   "Current state: ", int2string currentState1,
                      --   " final state ", int2string finalState12,
                      --   " nodelabel ", int2string nodeLabel1, 
                      --   " hostlabel ", int2string host1
                      --   ])) in
                      -- match p_map st301 printFunc currentState1 with (st301, partprint) in
                      -- match p_apply st301 partprint finalState12 with (st301, _) in
                      p_bind
                        st301
                        hrmStoreSubmodel
                        (hrmInit {})
                           ...
                           lam x456.
                             ...
                             then
                               ...
                               p_traverseSeq
                                 ...
                                 (match embeddedQMatrix12 with EmbeddedMarkovChainMatrix1 x4011
                                  then
                                    -- print "x4011.transitionProbs: ";
                                    -- dprint x4011.transitionProbs;
                                    -- printLn "---------------------";
                                    -- flushStdout;
                             ...
                             match
                               p_map
                                 ...
                                 (lam x465.
                                    lam x466.
                                      -- printLn (join ["st306:Previous value in chain: ", int2string x466]);
                                      -- printLn (join ["Distribution: ", strJoin ", " (map float2string (get x465 x466))]);
                             ...
                             in
                             match p_assume (join ["branchstate1 ", int2string nodeLabel1, " ", int2string host1, " ", float2string x456]) st309 (hrmStoreBranchState nodeLabel1 host1) x470 with (st310, nextState12)
                             in
                             match
                               p_traverseSeq
                                 st310
                             ...
                             in
                             match p_assume "dummy assume" st315 (hrmStoreBranchTime nodeLabel1 host1) x479 with (st316, x480)
                             in
                             match
                               p_map
                                 st316
              ...
              lam nodeLabel12.
                lam modelParams31.
                  lam x888.
                    ...
                    in
                    match p_assume (join ["anon17: ", int2string nodeLabel12, " ", int2string x888]) st575 (hrmStoreBranchTime nodeLabel12 x888) x898 with (st576, x899)
                    in
                    match
                      p_map
                        st576
                    ...
                    let st581 =
                      p_weight
                        st580
                        (hrmStoreBridgeSuppWeight nodeLabel12 x888)
                        (lam x907.
                           externalLog
                             (addf
              ...
              lam nodeLabel11.
                lam modelParams3.
                  lam x869.
                    ...
                    in
                    match p_assume (join ["anon15: ", int2string nodeLabel11, " ", int2string x869]) st563 (hrmStoreBranchTime nodeLabel11 x869) x878 with (st564, x879)
                    in
                    match
                      p_map
                        st564
                    ...
                    let st569 =
                      p_weight
                        st568
                        (hrmStoreBridgeSuppWeight nodeLabel11 x869)
                        (lam x887.
                           externalLog
                             (addf
                                       ...
                                       p_bind
                                         ...
                                         (lam st388.
                                            lam x614.
                                              ...
                                              in
                                              match
                                                p_subApply st402 hrmStoreSubmodel (hrmInit {}) x630 x632
                                              with
                                                (st403, x633)
                                              in
                                     ...
                                     p_bind
                                       ...
                                       (lam st454.
                                          lam x719.
                                            ...
                                            in
                                            match
                                              p_subApply st468 hrmStoreSubmodel (hrmInit {}) x735 x737
                                            with
                                              (st469, x738)
                                            in
  ...
  match
    p_assume "dummy assume"
      st hrmStoreLambda (p_pure (mkDirichlet [ 1., 1., 1., 1. ]))
  with
    (st499, lambda)
  in
  match
    p_assume "dummy assume" st499 hrmStoreMu (p_pure (mkExponential 10.))
  with
    (st500, mu)
  in
  match
    p_assume "dummy assume" st500 hrmStoreBeta (p_pure (mkExponential 1.))
  with
    (st501, beta)
  in
  ...
  match p_export st501 (hrmStoreTree (buildTree symbiontTree) (match input1 with {interactions = ints} in ints)) (p_pure ()) with st501 in
  match
    p_map
      st501
      (lam x788.
  ...
  in
  let nl =  (match postorderTree with MsgLeaf carried64
                     then
                       carried64.label
  ...
  ) mu with (st555, partres) in
  match p_apply st555 partres beta with (st555, partres) in
  match p_apply st555 partres lambda with (st555, partres) in
  match p_apply st555 partres (get rootRep 0) with (st555, partres) in
  match p_apply st555 partres (get rootRep 1) with (st555, res) in
  p_export
    st555
    hrmStoreExport
    res
lam st.
  ...
  in
  match p_map st528 (lam x. lam. printLn "dep arr"; x) #var"X13" with (st528, part_dep_arr) in
  match p_apply st528 part_dep_arr #var"1210" with (st528, dep_arr) in
  let qMatrix2 = {b14 with arr = dep_arr } in
  match p_map st528 (lam arr. printLn (strJoin ", " (map (lam i. float2string (externalExtArrGet arr i)) (range 0 9 1)))) dep_arr with (st528, _) in
  ...
  match p_apply st577 partres lambda with (st577, res) in

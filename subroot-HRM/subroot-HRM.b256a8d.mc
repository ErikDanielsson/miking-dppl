...
let g: all c10. all b30. all a88. (a88 -> b30 -> c10 -> a88) -> (a88, [b30]) -> c10 -> (a88, [b30]) =
  lam f32.
    lam acc19: (a88, [b30]).
      lam x212.
        match acc19 with (acc20, [ x189 ] ++ xs1)
        then
          (f32 acc20 x189 x212, xs1)
        else
    ...
    lam acc14: a86.
      lam seq11: [b28].
        lam seq21: [c8].
          ...
          then
            match foldl1 (g f30) (acc14, seq11) seq21 with (acc15, _)
            in
            acc15
          else match foldl1 (g1 f30) (acc14, seq21) seq11 with (acc16, _)
          in
          acc16
in
recursive
...
in
let anon4: Mat Float -> Mat Float = lam x185.
    x185 in
...
in
let anon6: Mat Float -> Mat Float = lam x184.
    x184 in
...
in
let anon8: Mat Float -> Mat Float = lam x183.
    x183 in
...
let anon9: (Float -> Float) -> Mat Float -> Mat Float -> Int -> () =
  lam f25.
    lam mtx13.
      lam mtx25.
        lam i19.
          extArrSetExn mtx25.arr i19 (f25 (extArrGetExn mtx13.arr i19))
in
let matMap: all x182. (Float -> Float) -> Mat Float -> Mat Float =
  lam f24: Float -> Float.
    lam mtx12: Mat Float.
      let mtx24 = matCopy mtx12 in
      let #var"12" = repeati (anon9 f24 mtx12 mtx24) (muli mtx12.m mtx12.n)
      in
      mtx24
in
    ...
    lam sums2.
      lam mtx23.
        lam i18.
          lam j1.
            matSetExn
              mtx23 i18 j1 (divf (matGetExn mtx11 i18 j1) (get sums2 i18))
in
let anon11: Mat Float -> [Float] -> Mat Float -> Int -> () =
  lam mtx10.
    lam sums1.
      lam mtx22.
        lam i17.
          repeati (anon10 mtx10 sums1 mtx22 i17) mtx10.n
in
  ...
  lam mtx9: Mat Float.
    let sums =
      extArrToSeq
        (matMulExn
           mtx9
           (matFromArrExn mtx9.m 1 (extArrOfSeq extArrKindFloat64 (make mtx9.m 1.)))).arr
    in
    let mtx21 = matCopy mtx9 in
    let #var"11" = repeati (anon11 mtx9 sums mtx21) mtx9.m in
    mtx21
in
...
let matExpRateSafe: Mat Float -> Mat Float =
  lam mtx8: Mat Float.
    matNormalizeRows (matMap anon12 (matExpExn mtx8))
in
let anon13: Matrix Float -> Int -> Float -> Float =
  lam t4.
    lam i16.
      lam acc12.
        addf acc12 (extArrGetExn t4.arr i16)
in
let matMean =
  lam t3.
...
let x: all #var"B13". all #var"A13". (Int -> #var"A13" -> #var"B13") -> Int -> #var"A13" -> #var"B13" =
  lam f22.
    lam i14.
      lam x180: #var"A13".
        let x181: #var"B13" = f22 (addi i14 1) x180 in
        x181
in
let x1: all #var"B12". all #var"A12". (#var"A12" -> #var"B12" -> #var"A12") -> #var"A12" -> #var"B12" -> #var"A12" =
  lam f21.
    lam a44.
      lam b11: #var"B12".
        let x179: #var"A12" = f21 a44 b11 in
        x179
in
let x2: all #var"B11". all #var"A11". (#var"A11" -> Int -> #var"B11" -> #var"A11") -> #var"A11" -> Int -> #var"B11" -> #var"A11" =
  lam f20.
    lam a43.
      lam idx11.
        lam b10: #var"B11".
          let x178: #var"A11" = f20 a43 (addi idx11 1) b10 in
          x178
in
...
let ifCont1 = lam #var"8": Int.
    0. in
let exp1: Float -> Float = lam x175: Float.
    mathExp x175 in
let log1: Float -> Float = lam x174: Float.
    mathLog x174 in
...
let anon15: all #var"X10". (Int -> #var"X10") -> Int -> #var"X10" =
  lam f17.
    lam idx9: Int.
      let x173: #var"X10" = f17 (addi idx9 1) in
      x173
in
...
let anon21: Float -> Float -> Float =
  lam sum1.
    lam x170: Float.
      let x171: Float = divf x170 sum1 in
      x171
in
...
let anon24: Int -> Int -> Float =
  lam index1.
    lam i12: Int.
      let x168: Float = bool2real (eqi index1 i12) in
      x168
in
...
let mtxGet: all #var"X2". Int -> Int -> Matrix #var"X2" -> #var"X2" =
  lam row1: Int.
    lam col: Int.
      lam mtx7: Matrix #var"X2".
        matGetExn mtx7 (subi row1 1) (subi col 1)
in
let mtxRowCols: all #var"X1". Matrix #var"X1" -> Int -> [Int] -> Matrix #var"X1" =
  lam mtx6: Matrix #var"X1".
    lam row: Int.
      lam cols: [Int].
        matRowCols mtx6 row cols
in
let mtxSclrMul: Float -> Matrix Float -> Matrix Float =
  lam scalar: Float.
    lam mtx5: Matrix Float.
      matScale scalar mtx5
in
let mtxTrans: Matrix Float -> Matrix Float = lam mtx4: Matrix Float.
    matTranspose mtx4
in
...
let anon25: Float -> Float -> Float =
  lam meanDist2.
    lam e1: Float.
      let x167: Float = divf e1 meanDist2 in
      x167
in
let anon26: Float -> [Float] -> [Float] =
  lam meanDist1.
    lam s5: [Float].
      let x166: [Float] = sapply s5 (anon25 meanDist1) in
      x166
in
...
let categoricalLogPmf: Int -> [Float] -> Float =
  lam x165: Int.
    lam param6: [Float].
      match
        match leqi 0 x165 with true
        then
          lti x165 (length1 param6)
        else
          false
      with
        true
      then
        log1 (get param6 (subi (addi x165 1) 1))
      else
        log1 0.
in
let anon27: Int -> Int -> Int = lam start15.
    lam idx7.
      addi idx7 start15
in
let anon28: Matrix Float -> Int -> Int -> Float =
  lam mtx1.
    lam i11.
      lam j: Int.
        let x164: Float = mtxGet i11 j mtx1 in
        x164
in
    ...
    lam i10: Int.
      seqNormalize
        (sapply
           (let start14 = 1 in
            let end8 = 3 in
            create1 (addi (subi end8 start14) 1) (anon27 start14))
           (anon28 mtx i10))
in
let anon29: Matrix Float -> Int -> Int -> Float =
  lam samplingProb3.
    lam h5: Int.
      lam obs: Int.
        let x163: Float = categoricalLogPmf obs (hostMsgToProb samplingProb3 h5)
        in
        x163
in
let anon30: Matrix Float -> Float -> Int -> Float =
  lam samplingProb2.
    lam acc9: Float.
      lam h4: Int.
        let x162: Float =
          let ph = hostMsgToProb samplingProb2 h4 in
          mulf acc9 (subf 1. (get ph (subi 3 1)))
        in
        x162
in
let anon31: Int -> Int -> Int = lam start13.
    lam idx6.
      addi idx6 start13
in
let getRepertoireSamplingDensity: [Int] -> Matrix Float -> Float =
  lam x161: [Int].
    lam samplingProb1: Matrix Float.
      let nHosts6 = length1 x161 in
      let logSamplingDensity1 = seqSumReal (sapplyi x161 (anon29 samplingProb1))
      in
      let probInvalidRep =
        fold
          (anon30 samplingProb1)
          1.
          (let start12 = 1 in
           let end7 = nHosts6 in
           create1 (addi (subi end7 start12) 1) (anon31 start12))
      in
      let logZReject = log1 (subf 1. probInvalidRep) in
      subf logSamplingDensity1 logZReject
in
let anon32: Int -> Int -> Int =
  lam acc8: Int.
    lam h3: Int.
      let x160: Int =
...
let updateRepertoire: [Int] -> Event -> Int -> [Int] =
  lam currRep8: [Int].
    lam event3: Event.
      lam nhosts7: Int.
        let hostIndex4 =
          let target95 = event3 in
          match target95 with Event1 x159
          then
            x159.host
          else
            let #var"2" =
        ...
        paste0
          [ slice currRep8 1 hostIndex4,
            [ let target94 = event3 in
              match target94 with Event1 x158
              then
                x158.toState
              else
                let #var"2" =
...
let anon33: Int -> Bool =
  lam i9: Int.
    ...
    x157
in
let anon34: Int -> Bool = lam i8: Int.
    let x156: Bool = eqi i8 2 in
    x156
in
      ...
      lam modelParams12: ModelParams.
        ...
        let baseRate1 =
          mtxGet
            ...
            (addi toState3 1)
            (let target92 =
               let target93 = modelParams12 in
               match target93 with ModelParams1 x155
               then
                 x155.embeddedQMatrix
               else
                 let #var"2" =
                   print
                 ...
                 exit 1
             in
             match target92 with EmbeddedMarkovChainMatrix1 x154
             then
               x154.mat
             else
               let #var"2" =
        ...
        then
          let currentHosts = whichTrue (sapply repertoire1 anon33) in
          let dist =
            mtxMean
              (mtxRowCols
                 (let target89 = modelParams12 in
                  match target89 with ModelParams1 x151
                  then
                    x151.hostMetric
                  else
                    let #var"2" =
            ...
            (pow
               ...
               (negf
                  (let target88 = modelParams12 in
                   match target88 with ModelParams1 x150
                   then
                     x150.beta
                   else
                     let #var"2" =
          ...
          let currentHosts1 = whichTrue (sapply repertoire1 anon34) in
          let dist1 =
            mtxMean
              (mtxRowCols
                 (let target91 = modelParams12 in
                  match target91 with ModelParams1 x153
                  then
                    x153.hostMetric
                  else
                    let #var"2" =
            ...
            (pow
               ...
               (negf
                  (let target90 = modelParams12 in
                   match target90 with ModelParams1 x152
                   then
                     x152.beta
                   else
                     let #var"2" =
    ...
    lam modelParams11.
      lam acc7: Float.
        lam i7: Int.
          ...
          x149
in
let anon36: Int -> Int -> Int = lam start11.
    lam idx5.
      addi idx5 start11
in
        ...
        then
          ...
          let baseRate =
            mtxGet
              ...
              (addi toState2 1)
              (let target86 =
                 let target87 = modelParams10 in
                 match target87 with ModelParams1 x148
                 then
                   x148.embeddedQMatrix
                 else
                   let #var"2" =
                     print
                   ...
                   exit 1
               in
               match target86 with EmbeddedMarkovChainMatrix1 x147
               then
                 x147.mat
               else
                 let #var"2" =
          ...
          baseRate
in
let anon37: [Int] -> ModelParams -> Float -> Int -> Float =
  lam currRep6.
    lam modelParams9.
      lam acc6: Float.
        lam i6: Int.
          ...
          x146
in
let anon38: Int -> Int -> Int = lam start10.
    lam idx4.
      addi idx4 start10
in
      ...
      lam nhosts6: Int.
        let gainRates =
          fold
            ...
            (let start9 = 1 in
             let end6 = nhosts6 in
             create1 (addi (subi end6 start9) 1) (anon36 start9))
        in
        let lossRates =
          fold
            (anon37 currRep5 modelParams8)
            0.
            (let start8 = 1 in
             let end5 = nhosts6 in
             create1 (addi (subi end5 start8) 1) (anon38 start8))
        in
  ...
  lam currRep4: [Int].
    lam nextEvent2: Event.
      lam modelParams7: ModelParams.
        let hostIndex1 =
          let target85 = nextEvent2 in
          match target85 with Event1 x145
          then
            x145.host
          else
            let #var"2" =
        ...
        match
          gti
            (let target83 = nextEvent2 in
             match target83 with Event1 x143
             then
               x143.fromState
             else
               let #var"2" =
            ...
            (let target84 = nextEvent2 in
             match target84 with Event1 x144
             then
               x144.toState
             else
               let #var"2" =
                  ...
                  then
                    ...
                    addf
                      ...
                      (hostIndepLikelihood
                         ...
                         embeddedQMatrix)
in
let anon39: [Int] -> [Int] -> Float -> Float -> [[Event]] -> ModelParams -> Float -> Int -> Float =
  lam fromRep2.
    lam toRep2.
      lam fromAge2.
        lam toAge2.
            ...
            lam modelParams6.
              lam acc5: Float.
                lam h2: Int.
                  let x141: Float =
                    ...
                    addf
                      ...
                      (hostIndepLikelihood
                         ...
                         toAge2
                         eventSeq5
                         (let target82 = modelParams6 in
                          match target82 with ModelParams1 x142
                          then
                            x142.embeddedQMatrix
                          else
                            let #var"2" =
                              print
                  ...
                  x141
in
let anon40: Int -> Int -> Int = lam start7.
    lam idx3.
      addi idx3 start7
in
        ...
        lam toAge1: Float.
          lam eventSeqs1: [[Event]].
            lam modelParams5: ModelParams.
              let unconditional1 =
                fold
                  (anon39 fromRep1 toRep1 fromAge1 toAge1 eventSeqs1 modelParams5)
                  0.
                  (let start6 = 1 in
                   let end4 = length1 eventSeqs1 in
                   create1 (addi (subi end4 start6) 1) (anon40 start6))
              in
              unconditional1
in
let anon41: Float -> Float -> Float =
  lam acc4: Float.
    lam val: Float.
      let x140: Float = addf acc4 val in
      x140
in
let anon42: Matrix Float -> Int -> Int -> Float =
  lam kernel1.
    lam fromState: Int.
      lam toState: Int.
        let x139: Float = log1 (mtxGet (addi fromState 1) (addi toState 1) kernel1)
        in
        x139
in
          ...
          lam eventSeqs: [[Event]].
            lam modelParams4: ModelParams.
              lam kernel: Matrix Float.
                ...
                in
                let logTotalTransProb = fold anon41 0. (zipWith1 (anon42 kernel) fromRep toRep)
                in
...
let simplexMove: [Float] -> Float -> Float -> Dist([Float]) =
  lam x137: [Float].
    lam alpha1: Float.
      lam epsilon: Float.
        let newParams = sapply x137 (newParamFun alpha1 epsilon) in
        Dirichlet newParams
in
let scaleMove: Float -> Float -> Dist(Float) =
  lam x136: Float.
    lam lambda6: Float.
      Reciprocal
        (mulf x136 (exp1 (divf (negf lambda6) 2.)))
        (mulf x136 (exp1 (divf lambda6 2.)))
in
let anon43: Int -> Int -> Int = lam start5.
    lam idx2.
      addi idx2 start5
in
let anon44: Int -> [Float] -> Float -> Float -> Int -> Float =
  lam x134.
    lam param5.
      lam currProb1.
        lam nextProb1.
    ...
    lam param4: [Float].
      lam lambda5: Float.
        lam errMargin1: Float.
          let currProb = get param4 (subi (addi x133 1) 1) in
            ...
            let newParam =
              sapply
                (let start4 = 1 in
                 ...
                 create1 (addi (subi end3 start4) 1) (anon43 start4))
                (anon44 x133 param4 currProb nextProb)
            in
...
let rbLambdaMove: [Float] -> Dist([Float]) =
  lam x132: [Float].
    ...
    let alpha = 25. in
    simplexMove x132 alpha _EPSILON
in
let rbBetaMove: Float -> Dist(Float) =
  lam x131: Float.
    let lambda4 = 1. in
    scaleMove x131 lambda4
in
let rbMuMove: Float -> Dist(Float) =
  lam x130: Float.
    let lambda3 = 0.2 in
    scaleMove x130 lambda3
in
let categoricalMove: Int -> [Float] -> Dist(Int) =
  lam x129: Int.
    lam param3: [Float].
      ...
      categoricalShiftKernel x129 param3 lambda2 errMargin
in
type ReturnType in
type SubrootInfo in
type Bridge in
  ...
  lam left4.
    lam right4.
      lam #var"7": Int.
        match
          geqf
            (let target80 = right4 in
             match target80 with Event1 x127
             then
               x127.eventTime
             else
               let #var"2" =
               ...
               in
               exit 1)
            (let target81 = left4 in
             match target81 with Event1 x128
             then
               x128.eventTime
             else
               let #var"2" =
  ...
  lam left3.
    lam right3.
      lam #var"6": Int.
        match
          isNaN1
            (let target79 = left3 in
             match target79 with Event1 x126
             then
               x126.eventTime
             else
               let #var"2" =
...
in
let anon45: Int -> Int -> Int = lam start3.
    lam idx1.
      addi idx1 start3
in
let anon46: [Float] -> Int -> Dist(Int) = lam param2.
    lam x125.
      categoricalMove x125 param2
in
let anon47: Matrix Float -> Int -> Int -> Int =
  lam msg1.
    lam nodeLabel6.
      lam h1: Int.
        let x123: Int =
    ...
    lam nHosts5: Int.
      lam nodeLabel5: Int.
        sapply
          (let start2 = 1 in
           let end2 = nHosts5 in
           create1 (addi (subi end2 start2) 1) (anon45 start2))
          (anon47 msg nodeLabel5)
in
let is2: Int -> Bool = lam x122: Int.
    eqi x122 2 in
    ...
    lam subrootLength1: Float.
      lam subrootLabel1: Int.
        lam qMatrix3: Matrix Float.
          lam nHosts4: Int.
            let subrootKernel = mtxExpRateSafe (mtxSclrMul subrootLength1 qMatrix3)
            in
            ...
            in
            let subrootRep = suggestNodeRep subrootSamplingProb nHosts4 subrootLabel1
            in
            let foo6 =
              weight
                (externalLog
...
let compAge: Event -> Event -> Int =
  lam left2: Event.
    lam right2: Event.
      match
        isNaN1
          (let target78 = right2 in
           match target78 with Event1 x121
           then
             x121.eventTime
           else
             let #var"2" =
...
in
let anon48: Int -> Int -> Int = lam start1.
    lam idx.
      addi idx start1
in
recursive
          ...
          lam host1: Int.
            lam nodeLabel4: Int.
              lam embeddedQMatrix5: EmbeddedMarkovChainMatrix.
                let totalRate1 =
                  get
                    (let target77 = embeddedQMatrix5 in
                     match target77 with EmbeddedMarkovChainMatrix1 x120
                     then
                       x120.totalRates
                     else
                       let #var"2" =
                  ...
                  weight
                    (externalLog
                       (addf
                          (addf
                             (bool2real
                                (let target76 = bridge in
                                 match target76 with Bridge1 x119
                                 then
                                   x119.success
                                 else
                                   let #var"2" =
                ...
                match target75 with Bridge1 x118
                then
                  x118.events
                else
                  let #var"2" =
                  ...
                  exit 1
in
let anon49: [Int] -> [Int] -> Float -> Float -> Int -> EmbeddedMarkovChainMatrix -> Int -> [Event] =
  lam startRep2.
    lam finalRep3.
      lam startAge2.
        lam finalAge5.
            ...
            lam nodeLabel2: Int.
              lam embeddedQMatrix3: EmbeddedMarkovChainMatrix.
                sapply
                  (let start = 1 in
                   let end1 = nHosts3 in
                   create1 (addi (subi end1 start) 1) (anon48 start))
                  (anon49
                     startRep1
                     finalRep2
                     startAge1
                ...
                lam branchKernel1: Matrix Float.
                  let unorderedBranch =
                    sampleUnorderedBranch
                      ...
                      nHosts2
                      nodeLabel1
                      (let target74 = modelParams3 in
                       match target74 with ModelParams1 x116
                       then
                         x116.embeddedQMatrix
                       else
                         let #var"2" =
                           print
                      ...
                      { age =
                        ...
                        history =
                          ...
                          then
                            ...
                            exit 1 }
in
let anon50: [Float] -> Dist([Float]) = lam lambda1.
    rbLambdaMove lambda1
in
let anon51: Float -> Dist(Float) = lam mu1.
    rbMuMove mu1 in
let anon52: Float -> Dist(Float) = lam beta1.
    rbBetaMove beta1
in
      ...
      lam hostDistances: [[Float]].
        lam subrootLength: Float.
          lam subrootLabel: Int.
            ...
            let subrootInfo =
              subrootNode
                (let target73 = postorderTree in
                 match target73 with MsgNode x114
                 then
                   x114.outMsg
                 else match target73 with MsgLeaf x115
                 then
                   x115.outMsg
                 else
                   let #var"2" =
            ...
            let rootSamplingProb =
              mtxElemMul
                (let target71 = subrootInfo in
                 match target71 with SubrootInfo1 x111
                 then
                   x111.msg
                 else
                   let #var"2" =
            ...
            let rootRep =
              suggestNodeRep
                rootSamplingProb
                nHosts1
                (let target70 = postorderTree in
                 match target70 with MsgNode x109
                 then
                   x109.label
                 else match target70 with MsgLeaf x110
                 then
                   x110.label
                 else
                   let #var"2" =
            ...
            let subrootBranchSample =
              sampleBranch
                ...
                (let target66 = postorderTree in
                 match target66 with MsgNode x102
                 then
                   x102.label
                 else match target66 with MsgLeaf x103
                 then
                   x103.label
                 else
                   let #var"2" =
                   ...
                   exit 1)
                modelParams2
                (let target67 = subrootInfo in
                 match target67 with SubrootInfo1 x104
                 then
                   x104.kernel
                 else
                   let #var"2" =
...
in
let anon53: {interactions: [[Int]], subrootLabel: Int, symbiontTree: TreeLabeled, hostDistances: [[Float]], subrootLength: Float} -> () -> ReturnType =
  lam input1.
    lam #var"5".
      hostRepertoireModel
        input1.symbiontTree

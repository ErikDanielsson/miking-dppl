  ...
  sem run = | st -> 
    ...
    match (switch stree 
      ...
      case SNode n then
        ...
        -- Run the recursion
        match p_bind_ st #frozen"walk" left (PVHCons (gtrans, PVHCons (gtime, PVHNil ()))) with (st, res) in
        match p_bind_ st #frozen"walk" right (PVHCons (gtrans, PVHCons (gtime, PVHNil ()))) with (st, res) in
        (st, res)
    end) with (st, res) in
    match p_map st ((flip get) 0) params with (st, l01) in
    p_export st simpleExport l01
...
lang TreeHistoryReconstruction = SimpleResample
  sem run = | st -> 
    ...
    let clamped_branch : all z.
      PValHList z Unknown -> PValState () z -> PVal z ((Int, Float), Int) -> PValState () z =
      lam list. lam st. lam s. 
        match p_map st (lam s. s.0) s with (st, s_) in
        match p_bind_ st #frozen"branch" s_ list with (st, e) in
    ...
    let zipWithDetPVal_ : all z. all a. all b. all c.   
      PValState () z ->  [a] -> [PVal z b] -> (a -> b -> c) -> (PValState () z, [PVal z c]) =
    ...
    recursive let walk : all z.
      PValState () z -> (MsgTree, Float, Mat Float, [Int]) -> PValHList z Unknown -> (PValState () z, PVal z Unknown) =
      lam st. lam arg. lam list. 
      ...
      switch msgTree 
        ...
        case MsgNode n then 
         match combineMsgs msg n.msg with (msg, _) in
          ...
          match p_bind_ st #frozen"walk" left list with (st, _) in
          ...
          p_bind_ st #frozen"walk" right list 
      end
    in
    -- Compute the postorder messages
    ...
    let init : all z.
      PValState () z -> (Mat Float, Float, Float, MsgTree) -> PValHList z Unknown -> (PValState () z, PVal z Unknown) =
      lam st. lam msgData. lam list.
      ...
      switch msgTree 
        ...
        case MsgNode n then
          ...
          match p_bind_ st #frozen"walk" left list with (st, res) in
          -- Right
          ...
          match p_bind_ st #frozen"walk" right list with (st, res) in
          (st, res)
      end
    in
    let list = PVHCons (gtrans, PVHCons (gtime, PVHNil ())) in
    match p_bind_ st #frozen"init" msgData list with (st, res) in
    match p_map st ((flip get) 0) params with (st, l01) in
    p_export st simpleExport l01
  end

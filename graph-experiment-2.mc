...
lang PValInterface
  ...
  -- Check whether the current value produced by the given weight
  -- is valid
  sem checkPreviousWeight : all complete. all st. PWeightRef -> PValInstance complete st -> Bool
...
lang MutPVal = PValInterface
  ...
  syn PWeightRef = | PWeightRef { checkValid : () -> Bool } -- TODO(vipa, 2025-09-23): figure out what we want to be able to do here, and thus what we need to store
  ...
  sem instantiate f = | st ->
    print "Initalizing...: ";
    recursive let rejinit = lam i. 
      match _initModel 0 st f with (st, initWeight, update) in
      if eqf initWeight (negf inf) then
        (print (join [int2string i, ", "]));
        rejinit (addi i 1) 
      else PVI {st = st, update = update, permanentWeight = initWeight, id = 0} in
    let s = rejinit 0 in
    printLn "";
    s
  ...
  sem checkPreviousWeight wref = | _ -> 
...
lang SimplePersistentPVal2 = PValInterface
  ...
  syn PWeightRef = | PWeightRef { checkValid : PA Dyn -> Bool } -- TODO(vipa, 2025-09-29):
  ...
  sem checkPreviousWeight wref = | PVI x ->
    match wref with PWeightRef w in
    true -- a.readValue x.state.values
  ...
  | PVal x ->
    ...
    mapSt (lam st. store st (PWeightRef { checkValid = lam. true })) st
  | PVar x ->
    let readValue = mkGetState st x.id in
    ...
    let st = mapSt (lam st. store st (PWeightRef { checkValid = lam. true })) st in
...
let p_reciprocal : Float -> Float -> PDist Float
  = lam a. lam b.
    { sample = lam. reciprocalSample a b
    , logObserve = lam x. reciprocalLogPdf a b x
    }
...
let bucket : all a. Int -> Float -> Float -> [Float] -> [((Float, Float), Float)]
  = lam numBuckets. lam min. lam max. lam l.
    let n = int2float (length l) in
    let l = filter (lam s. and (geqf s min) (leqf s max)) l in
    let n_bucket = int2float (length l) in
    let prop_bucket = divf n_bucket n in
...
type SomePWeightRef
con SomePWeightRef : all x. use PValInterface in PWeightRef -> SomePWeightRef

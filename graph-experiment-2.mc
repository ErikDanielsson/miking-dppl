...
lang CTMCBridge = SimpleResample
  sem run = | st -> 
    ...
    match p_map st (lam params. get params 0) params with (st, l01) in
    match p_map st (lam params. get params 1) params with (st, l10) in
    match p_map st (lam params. get params 2) params with (st, l12) in
    match p_map st (lam params. get params 3) params with (st, l21) in
    match p_map st (lam l01. lam l10. lam l12.lam l21. [l01, addf l10 l12, l21]) l01 with (st, _rates3) in
    match p_apply st _rates3 l10 with (st, _rates2) in
    match p_apply st _rates2 l12 with (st, _rates1) in
    match p_apply st _rates1 l21 with (st, rates) in
    match p_map st get rates with (st, grate) in
    -- Construct embedded Markov chain matrix
    match p_map st (lam rates. get rates 1) rates  with (st, rate1) in
    match p_map st (lam l10. lam l12. lam r1. [
      [0., 1., 0.], 
      [divf l10 r1, 0., divf l12 r1], 
      [0., 1., 0.]
    ]) l10 with (st, eMC) in
    match p_apply st eMC l12 with (st, eMC) in
    match p_apply st eMC rate1 with (st, eMC) in
    match p_map st get eMC with (st, geMC) in
    -- Construct the CTMC recursion
    recursive let ctmc : all z.
      PValState () z -> Unknown -> PValHList z Unknown -> (PValState () z, PVal z Unknown)
      = lam st. lam s. lam list.
      match list with PVHCons (geMC, PVHCons (grate, PVHNil ())) in
      match s with (a, t) in
      if geqf t 0. then
        match p_pure st a with (st, a_) in
        -- Draw the next state
        match p_apply st geMC a_ with (st, params) in
        match p_map st p_categorical params with (st, dist_a) in
        match p_assume_ st dist_a with (st, b_) in
        -- Draw the next time
        match p_apply st grate a_ with (st, r) in
        match p_map st p_exponential r with (st, dist) in
        match p_assume_ st dist with (st, t0) in
    ...
    match p_apply st grate a_ with (st, rate) in
    match p_map st p_exponential rate with (st, dist_t) in
    ...
    match p_bind_ st #frozen"ctmc" s (PVHCons (geMC, PVHCons (grate, PVHNil ()))) with (st, res) in
